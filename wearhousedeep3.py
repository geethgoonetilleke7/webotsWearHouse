...
class MotionController:
    ...
    def execute_turn_until_aligned(self, turn_type):
        """Hybrid approach: fixed rotation delay + sensor alignment"""
        ...
        # STEP 1: Fixed rotation delay to ensure we leave current line
        print("Step 1: Fixed rotation to leave current line...")
        
        if turn_type == 'LEFT':
            self.set_mecanum_velocity(0, 0, 2.0)  # CCW rotation
            fixed_rotation_steps = 400  # 10 steps fixed rotation
        elif turn_type == 'RIGHT':
            self.set_mecanum_velocity(0, 0, -2.0)  # CW rotation
            fixed_rotation_steps = 400  # 10 steps fixed rotation
        elif turn_type == 'BACK':
            self.set_mecanum_velocity(0, 0, 2.0)  # CCW rotation
            fixed_rotation_steps = 140  # 20 steps fixed rotation for 180
        
        # Execute fixed rotation steps
        step_count = 0
        while step_count < fixed_rotation_steps:
            self.robot.step(self.timestep)
            step_count += 1
        
        print(f"Fixed rotation completed ({fixed_rotation_steps} steps)")
        
        # STEP 2: Continue rotating until middle sensors align with new line
        print("Step 2: Sensor-based alignment...")
        
        aligned = False
        timeout_steps = 500  # Maximum steps for alignment (safety)
        step_count = 0
        consecutive_aligned = 0
        required_consecutive = 2  # Require 2 consecutive readings for stability
        
        while not aligned and step_count < timeout_steps:
            self.robot.step(self.timestep)
            step_count += 1
            
            # Check middle sensors for line alignment
            sensor_values = self.sensor_manager.read_line_sensors()
            middle_sensors = sensor_values[4:7]  # D4, D5, D6
            
            # Count how many middle sensors see the line
            middle_on_line = sum(1 for v in middle_sensors if v == 0)
            
            # Debug output occasionally
            if step_count % 10 == 0:
                print(f"Alignment check - Middle sensors: {middle_on_line}/3")
            
            # If 2 or 3 middle sensors detect the line consistently
            if middle_on_line >= 2:
                consecutive_aligned += 1
                if consecutive_aligned >= required_consecutive:
                    aligned = True
                    print(f"Alignment successful! Middle sensors: {middle_on_line}/3")
            else:
                consecutive_aligned = 0  # Reset counter
        
        # Stop rotation
        self.stop()
        
        if aligned:
            print(f"Turn completed successfully in {step_count} steps")
        else:
            print(f"Turn timeout after {step_count} steps - forcing completion")
        
        # STEP 3: Small forward movement to ensure proper alignment
        print("Step 3: Final alignment forward move...")
        self.set_mecanum_velocity(0, 1.5, 0)
        forward_steps = 0
        while forward_steps < 8:  # 8 steps forward
            self.robot.step(self.timestep)
            forward_steps += 1
        self.stop()
...
class NavigationSystem:
    ...
    def handle_intersection(self, inter_type):
        ...
        if self.motion.execute_turn_until_aligned(action):
            # Update direction after successful turn
